...
interface VenueListProps {
    data: MenuItem[];
    opportunityData?: MenuItem[];
    title?: string;
}

const VenueList: FC<VenueListProps> = ({ data, opportunityData, title = "Venue List" }) => {
    const [viewMode, setViewMode] = useState<'current' | 'opportunity'>('current');
    const [sortField, setSortField] = useState<keyof MenuItem>('insegna');
    ...
    const downloadCSV = () => {
        const headers = ['Venue Name', 'City', 'Region', 'Type', 'Brand', 'Cocktail', 'Price'];
        ...
        link.setAttribute("download", `venue_list.csv`);
        ...
        <div className="bg-gray-800 rounded-lg shadow-lg border border-gray-700 overflow-hidden">
            <div className="p-4 border-b border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                <h3 className="text-lg font-semibold text-gray-200">
                    {viewMode === 'current' ? title : `Growth Opportunities`}
            ...
            {activeData.length === 0 ? (
                <div className="p-8 text-center text-gray-400">
                    {viewMode === 'current'
                        ...
                        : "No growth opportunities found in this area."}
                </div>
            ) : (
                    ...
                    <table className="w-full text-left text-sm text-gray-300">
                        <thead className="bg-gray-900/50 text-xs uppercase text-gray-400">
                            <tr>
                                ...
                                <th className="px-6 py-3 cursor-pointer hover:text-teal-400" onClick={() => handleSort('brand')}>Brand</th>
                                <th className="px-6 py-3 cursor-pointer hover:text-teal-400" onClick={() => handleSort('nomeCocktail')}>Cocktail</th>
                                <th className="px-6 py-3 text-right cursor-pointer hover:text-teal-400" onClick={() => handleSort('prezzo')}>Price</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-700">
                            {paginatedData.map((item, index) => (
                                <tr key={`${item["Menu ID"] || index}-${index}`} className="hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-teal-300">{item.brand}</td>
                                    <td className="px-6 py-4">{item.nomeCocktail || '-'}</td>
